...
Besides the standard "PC Mode" describes above, Keymmodore-64 also includes four modes optimized for all 8 bit Commodore Computers emulators, like VICE or BMC64. Using these modes, you can easily use your C-64 keyboard not even emulating a Commodore-64 or VIC-20, but also a Commodore-128, a Plus/4 (or C-16), and even PET Computers with Graphics or Business keyboards.
...
The C-64 keyboard matrix is connected directly to the pins of the Pi Pico. However if you want to add Atari-like joysticks to your project, you need to add some wiring in between the keyboard connector and the Pico. As you can see in the matrix diagram below, P1 and P2 use the "I" file and the "8" row of the RESTORE key. Remember, THIS IS OPTIONAL (just because there are some free crosspoints). If your are not interested on those old joysticks, or you plan to use a Raspberry (like a Pimmodore-64), you do not need to add this extra complexity.
